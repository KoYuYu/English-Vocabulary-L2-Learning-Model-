...
{ 
    for ($i=0; $i<sizeof($checkcalss); $i++)
    {        
		//$sql = "delete from `class` where `classname` = '".$checkcalss[$i]."'";
        //$result = $conn -> query($sql) or die("無法執行：".mysqli_error($conn)); 
		//$sql = "delete from `classword_$id` where `classname` = '".$checkcalss[$i]."'";
        //$result = $conn -> query($sql) or die("無法執行：".mysqli_error($conn)); 
        //$sql = "Update from `classword_$id` where `classname` = '".$checkcalss[$i]."'";
        //$result = $conn -> query($sql) or die("無法執行：".mysqli_error($conn)); 
		if ($newclass ==null){
	...
	echo "<script>alert('delete 成功！將回到所有課程頁面'); location.href = 'class_delete1.php';</script>";
...
<?php
    ...
    echo "<td><input type='checkbox' name='voc[]' value='$row[0]' $checked></td>";
    echo "<td >" . $row[1] . "</td>";
    echo "<td >" . $row[2] . "</td>";
