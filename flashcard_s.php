...
<!--Header End Here--> 
  ...
  <body>
   <center>
	...
	<?php
			$id = $_SESSION['email'];
			$sql= "SELECT * FROM `search_record`  WHERE `member`='$id' LIMIT 1";
//$sql="SELECT name FROM `member` WHERE `email`='$id';";
			$result = $conn -> query($sql) ;
			$row=$result -> fetch_array();
			$snum=$row[0];
			$word=$row[2];
			$meaning=$row[3];
			$sql="INSERT INTO `flashcard_record_$id` (`english`, `meaning`, `time` , `source`) VALUES ('$word', '$meaning', now() , 'searched')";
			$result = $conn -> query($sql) ;
...
<?php 
if(isset($_POST['prev'])){
$sql= "SELECT * FROM  `search_record` WHERE  `member` =  'a' AND  `searchnum` <".$snum." LIMIT 1";
$result = $conn -> query($sql) ;
$row=$result -> fetch_array();
$snum=$row[0];
$word=$row[2];
...
<input type="button" value="next" align="right" class="button" onclick="javascript:history.go(-1)"> 
</center>
