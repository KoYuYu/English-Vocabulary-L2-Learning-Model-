...
<?php 
$str="SELECT * FROM `member` where `identity`='student'";
...
{       
        $sql="SELECT * from `member` where `identity`='student' and `id`= ".$_GET['act']."";
		$result = $conn->query($sql);
        $row=$result -> fetch_array();
        echo "<table border=2 align='center' width='20px', height='20px'>";
		echo "<tr><th colspan='3'>$row[1]</th></tr>";
		$sql="SELECT * from quiz_600_record where";
		$res = $conn->query($sql);
		$sql="SELECT `english` as 單字,  SUM(if(quiz_600_record.correct_time = '1', 1, 0)) as 正確次數, SUM(if(quiz_600_record.error_time = '1', 1, 0)) as 錯誤次數 FROM `quiz_600_record` WHERE `member`= '$row[2]'  GROUP BY `english` order by `quiznum` DESC limit 0, 20;";
		$result = $conn -> query($sql) ;
