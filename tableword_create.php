...
<td width="150" cellpadding="1" cellspacing="3">課程</td>
	<input name="classname" type="text" size="12">
<table id="mytable" width="580" border="2" align="center" cellpadding="1" cellspacing="1" >
  <tr>
    <td width="150" class="td01">英文</td>
	<td width="200" class="td01">詞性</td>
    <td width="200" class="td01">解釋</td>
  </tr>
  <tr>
    <td>
    <input name="name[]" type="text" onKeyPress="return FormEnterKey(event)" size="12">
    </td>
	 <td>
    <input name="speech[]" type="text" onKeyPress="return FormEnterKey(event)" size="12">
    </td>
    <td>
    <input name="content[]" type="text" onKeyPress="return FormEnterKey(event)" size="12">
    </td>
  </tr>
  <tr>
    <td>
...
$engspeech = $_GET['speech'];

if($_GET['submit']=="確認")
{   
    for($i=0; $i<sizeof($engword); $i++)
    {
        $sql = "select * from `iBT_TOEFL` where `English` = '".$engword[$i]."'";
        $result = $conn -> query($sql) or die("無法執行：".mysqli_error($conn)) ;
        $row1 = $result -> fetch_array();
        $oldexpl=$row1[3];
        $newexpl="$oldexpl,$engexpl[$i]";
        //echo $newexpl;
        $oldclassi= $row1[4];
        $newclassi= "$oldclassi,$classname";
        //echo $newclassi;
        if($engword[$i] != $row1[1])
        {
            $sql = "insert into `iBT_TOEFL` (`English`,`speech`, `Chinese`, `classified`) VALUES ('".$engword[$i]."','".$engspeech[$i]."', '".$engexpl[$i]."', '".$classname."')";
        
//(行星的)組態；(原子的)排列組合(常考) Astronomy  //on duplicate key update `chinese`= `chinese` + ',' + '".$engexpl[$i]."'
            $result = $conn -> query($sql) or die("無法執行：".mysqli_error($conn)) ;
        }
        else
        {
            $sql = "update `iBT_TOEFL` set `chinese`='$newexpl', `classified` = '$newclassi' where `English`='".$engword[$i]."'";
            $result = $conn -> query($sql) or die("無法執行：".mysqli_error($conn)) ;
        }
    ...
    echo "window.location.href='tableword_create.php'";
    echo "</script>";   
}
?>
...
function add_new_data() {
 ...
 Td = Tr.insertCell(Tr.cells.length);
 Td.innerHTML='<input name="speech[]" type="text" size="12">';
 Td = Tr.insertCell(Tr.cells.length);
