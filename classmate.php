...
$sql="select * from `member1` where `name` != '$id' ";
...
<!--<div class="CSSTableGenerator" >-->
     <a href="classRoom1.php?ord=<?php echo $class; ?>"><img src="images/back_icon.png" title="回到此課程頁面" border="0" width="70px" height="50px" ></a><br>

<table border="1" style="margin-left:auto;margin-right:auto">
	<th colspan="3"><?php echo "$classEng";?> </th>
	<tr>
		<td>選取</td>
		<td>姓名</td>
        <td>帳號</td>
...
<p align="center">
    <input type="submit" name="submit" value="勾選會員加入到此課程" class="button"> 
...
<?php
    ...
    //echo "<td><input type='hidden' name='voc[]' value='$row[1]' $checked></td>";
    echo "<td >" . $row[1] . "</td>";
    echo "<td >" . $row[2] . "</td>";
...
{
    $sql = "update `classword1_$id` SET `marked` = 0 where `classname`='$class'";
    $result = $conn -> query($sql) or die("無法執行：".mysqli_error($conn)); 
	for($i=0; $i<sizeof($checkvoc); $i++)
    {
        $sql = "update `classword1_$id` SET `marked` = 1 where `classname`='$class' and `english` = '".$checkvoc[$i]."'";
        $result = $conn -> query($sql) or die("無法執行：".mysqli_error($conn));          
    }
