...
<!--Header End Here--> 
  ...
  <body>
   <center>
	...
	<?php
			$id = $_SESSION['email'];
			$sql= "SELECT * FROM `save_600_record` WHERE `member`='$id' ORDER BY RAND() LIMIT 1";
			$result = $conn -> query($sql) ;
			$row=$result -> fetch_array();
			$word=$row[1];
			$meaning=$row[2];
			$sql="INSERT INTO `flashcard_record_$id` (`english`, `meaning`, `time` , `source`) VALUES ('$word', '$meaning', now() , 'favorite')";
			$result = $conn -> query($sql) ;
